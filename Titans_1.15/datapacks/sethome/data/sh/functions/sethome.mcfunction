...
execute if score #sh_limit sh_limit matches 1.. run tellraw @s ["",{"text":"Home set! Sethomes remaining: ","color":"yellow"},{"score":{"name":"@s","objective":"sh_limitShow"},"color":"red"}]
execute if score #sh_limit sh_limit matches ..0 run tellraw @s ["",{"text":"Home set!","color":"yellow"}]
